...
$(document).ready(function () {
    ...
    //    $("#div_question_js").fadeIn();
    //    $("#btn_start_chat").fadeOut();
    //});
...
function ToggleFormContact(remark) {
    ...
    html = `
     <div class="icon_chat_bot_sitto_append">
         ...
         <div class="container div_form_submit_wrapper">
...
function GetListProduct(id, contents) {
    $.ajax({
        ...
        success: function (response) {
            ...
            var listData = response.data;
            if (listData != null && listData.length > 0) {
                var buttonHtml = '', productHtml = '', text = '';
                    ...
                    switch (parseInt(item.isContact)) {
                        case 0:
                            if (!IsNullOrEmty(item.url)) {
                                ...
                                //san-pham/${metaUrl}
                                buttonHtml = `<a href="/san-pham/${item.productObj?.metaUrl}" target="_blank" rel="noreferrer" style="background: #17a2b8;" class="btn mb-1 text-white mx-auto button_chat_bot_item">Chi tiết</a>`
                            }
                            break;
                        case 1:
                            //Bật form
                    ...
                    <div class="mx-2 bg-white">
                        <div style="padding: unset !important;" class="product-box-4 product_image_chat_bot">
                            <div style="height: 200px;" class="product-image product-image-2">
                                <a /san-pham/${item.productObj.metaUrl}/${item.productObj?.metaUrl}>
                            ...
                            <div style="height: 50px" class="p-2 product-detail mt-1">
                                <a data-bs-toggle="tooltip" data-bs-placement="top" title="${item.productObj?.name}" href="/san-pham/${item.productObj.metaUrl}/${item.productObj?.metaUrl}">
