var $categoryIdLv1 = $('#input_categoryLv1_id'),
    $categoryIdLv2 = $('#input_categoryLv2_id'),
    $pageEl = $('#input_page'),
...
const dataParms = function () {
    ...
    var c2 = !IsNullOrEmty($categoryIdLv2.val()) ? $categoryIdLv2.val() : $(".btn_tab.active").val();
    return {
        c2 : c2,
        c1: parseInt($categoryIdLv1.val()),
                ...
                if (listData != null && listData.length > 0) {
                    ...
                    $.each(listData, function (key, value) {
                        ...
                        tmpHtml +=
                            ...
                            `  <div>
                                    <div class="product-box-3 h-100 wow fadeInUp bg-white" style="border: 1px solid #ececec">
                                        <div class="">
                                            <div class="product-image knowledge_new_modifile">
                                                <a href="/kien-thuc/${value.categoryObj?.metaUrl}/${value.metaUrl}">
                                                    <img data-src="${IsNullOrEmty(value.imageObj?.smallUrl) ? "/img_dev/error/product.png" : value.imageObj?.smallUrl}"
                                                         class="img-fluid blur-up lazyload h-100 w-100 div_css_item_img" alt="${value.title}"
                                                         onerror="this.onerror=null;this.src='/img_dev/error/avatar.png';">
                                                </a>
                                            </div>
                                        </div>
                                        <div class="product-footer">
                                            <div class="product-detail p-1 div_item_know_css">
                                                ...
                                                <span class="knowledge_category_item category_clamp_title text-uppercase">${value.categoryObj?.name}</span>
                                                <a data-bs-toggle="tooltip" data-bs-placement="top" title="${value.title}" href="/kien-thuc/${value.categoryObj?.metaUrl}/${value.metaUrl}">
                                                    <h5 class="name_clamp mt-2 font_item_modifeil knowledge_title_item_custom">${value.title}</h5>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
